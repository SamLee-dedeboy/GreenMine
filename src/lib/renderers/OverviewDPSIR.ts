...
export const OverviewDPSIR = {
  init(svgId: string) {
    ...
    this.dispatch = d3.dispatch("VarTypeLinkSelected");
...
//   },
  ...
  drawLinks(
    ...
    const pairInfo = Object.values(mappedLinks.reduce((acc, curr) => {
    ...
    const lineGenerator = (link) => {
        ...
        const givenPoint = grid_layout.gridToSvgCoordinate(150, 120, cellWidth, cellHeight);
        ...
        const dotProduct = (givenPoint.x - sourcePoint.x) * normalizedVectorX + 
    ...
    const link_paths = svg
      ...
      .join("path")
      .attr("class", "link")
      .attr("id", (d:tLinkObjectOverview) => `${d.source}` + "-" + `${d.target}`)
