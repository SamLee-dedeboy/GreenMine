...
export const simgraph = {
  ...
  init(svgId, width, height, paddings, handlers) {
    ...
    const svg = d3
      ...
      .on("click", function (e) {
        if (!e.defaultPrevented) {
          ...
          d3.selectAll("text.topic-label").classed(
    ...
    this.xScale_keywords = d3
      .scaleLinear()
      .domain([0, 1])
      .range([0, this.keyword_region_size.width]);
    this.yScale_keywords = d3
      .scaleLinear()
      .domain([0, 1])
      .range([0, this.keyword_region_size.height]);
  ...
  update_chunks(groups, group_ccs, emotionColorScale) {
    const group_node_sizes = Object.keys(groups).map(
      (group_id) => groups[group_id].length,
    );
    const bbox_min_max_size = [
      d3.min(group_node_sizes),
      d3.max(group_node_sizes),
    ];
    const bboxRadiusScale = d3
      .scaleLinear()
      .domain(bbox_min_max_size)
      .range([150, 280]);
    const bboxes = radialBboxes(Object.keys(groups), this.width, this.height, {
      width: 200,
      height: 200,
    });
    const chunk_region = d3.select("#" + this.svgId).select("g.chunk_region");
    ...
    chunk_region
      ...
      .each(function (d) {
        ...
        group.selectAll("*").remove();
        const bbox_size = bboxRadiusScale(groups[d].length);
        console.log({ bboxes, d });
        const bbox_center = bboxes[d].center;
        const bbox_origin = [
          bbox_center[0] - bbox_size / 2,
          bbox_center[1] - bbox_size / 2,
        ];
        const bboxCoordinateScaleX = d3
          .scaleLinear()
          .domain([0, 1])
          .range([bbox_origin[0], bbox_origin[0] + bbox_size]);
        const bboxCoordinateScaleY = d3
          .scaleLinear()
          .domain([0, 1])
          .range([bbox_origin[1], bbox_origin[1] + bbox_size]);
        const bbox = group
          .append("rect")
        ...
        const collisionForce = d3
          .forceSimulation(groups[d])
          .alphaMin(0.1)
          .force("collide", d3.forceCollide(node_radius))
          .force(
            "x_force",
            d3.forceX((d) => bboxCoordinateScaleX(d.coordinate[0])),
          )
          .force(
            "y_force",
            d3.forceY((d) => bboxCoordinateScaleY(d.coordinate[1])),
          )
          .on("tick", () => {
            nodes
              .attr(
                ...
                (d) =>
                  (d.x = clip(d.x, [
                    bbox_center[0] - bbox_size / 2 + node_radius,
                    bbox_center[0] + bbox_size / 2 - node_radius,
                  ])),
              )
              .attr(
                ...
                (d) =>
                  (d.y = clip(d.y, [
                    bbox_center[1] - bbox_size / 2 + node_radius,
                    bbox_center[1] + bbox_size / 2 - node_radius,
                  ])),
              );
            let cc_paths: any[] = [];
  ...
  update_keywords(keyword_data, _) {
    ...
    const hexbin = Hexbin()
      ...
      .extent([
        [0, 0],
        [this.keyword_region_size.width, this.keyword_region_size.height],
      ]);
    ...
    //     .domain([0, d3.max(data_bins, d => d.length)])
    //     .range([0, 1]);
    const binSumFrequency = (bins) =>
      d3.sum(
        bins,
        (keyword) => keyword_data.keyword_statistics[keyword].frequency,
      );
    const binMaxFrequency = d3.max(data_bins, binSumFrequency);
    const scaleColor = d3.scaleSequential(
      [0, Math.sqrt(binMaxFrequency)],
      d3.interpolateBlues,
    );
      ...
      .forEach((keyword) => {
        ...
        if (hex_labels[closest_hex_index] != null) {
          const previous_label_freq =
            keyword_statistics[hex_labels[closest_hex_index]].frequency;
    ...
    group
      ...
      .attr("y", (d) => d.y)
      .attr("fill", (d) => {
        return Math.sqrt(binSumFrequency(d) / binMaxFrequency) > 0.65
          ? "white"
          : "black";
      })
...
function force_layout(
  ...
  const forceLink = d3
    .forceLink(group_links)
    .id((d) => d.id)
    .distance((d) => d.weight);
  const simulation = d3
    .forceSimulation(group_nodes)
    .force("link", forceLink)
    .force("charge", forceNode.distanceMin(10))
    .force(
      "center",
      d3.forceCenter(group_center[0], group_center[1]).strength(0.1),
    )
    .force("collide", d3.forceCollide(node_radius))
    .alphaMin(0.01)
    .on("tick", () => {
      nodes
        .attr(
          ...
          (d) =>
            (d.x = clip(d.x, [
              group_bbox.x1 + node_radius + 1,
              group_bbox.x2 - node_radius - 1,
            ])),
        )
        .attr(
          "cy",
          (d) =>
            (d.y = clip(d.y, [
              group_bbox.y1 + node_radius + 1,
              group_bbox.y2 - node_radius - 1,
            ])),
        );
    ...
    for (let j = 0; j < nodes.length; j++) {
      ...
      if (weights[nodes[i].id] && weights[nodes[i].id][nodes[j].id])
        force_links.push({
          source: nodes[i].id,
          target: nodes[j].id,
          weight: weights[nodes[i].id][nodes[j].id],
          type: "real",
        });
      // else
...
function collision_detection(
  ...
  bbox_size,
  node_radius,
  bboxCoordinateScaleX,
  bboxCoordinateScaleY,
) {
  const collisionForce = d3
